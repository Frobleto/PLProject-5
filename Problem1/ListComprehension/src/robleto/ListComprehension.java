...
public class ListComprehension {
    public static void main(String[] args) {
        ...
        emp.add(e24);
        emp.add(e25);
        System.out.println("SELECT * FROM emp WHERE last_name = 'BROWN'");
        emp.stream()
                .filter(e -> e.get(1).equals("BROWN"))
                .forEach(e -> { System.out.println(e); });

        System.out.println();

        System.out.println("SELECT salary FROM emp WHERE salary > 2000 ORDER BY salary");
        emp.stream()
                .map(e -> e.get(7))
                .filter(e -> (int) e > 2000)
                .sorted()
        ...
        System.out.println("SELECT * FROM emp WHERE title = 'STOCK CLERK ORDER BY dept_id");
        emp.stream()
                .filter(e -> e.get(6).equals("STOCK CLERK"))
                .sorted((a, b) -> Integer.compare((int) a.get(9),(int) b.get(9)))
                .forEach(e -> {System.out.println(e);});

        System.out.println();

        System.out.println("SELECT * FROM emp WHERE title = 'SALES REPRESENTATIVE ORDER BY dept_id desc");
        emp.stream()
                .filter(e -> e.get(6).equals("SALES REPRESENTATIVE"))
                .sorted((a, b) -> Integer.compare((int) b.get(9),(int) a.get(9)))
                .forEach(e -> {System.out.println(e);});

        System.out.println();

        System.out.println("SELECT * FROM emp ORDER BY manager_id");
        emp.stream()
                .sorted((a, b) -> Integer.compare((int) a.get(10), (int) b.get(10)))
                .forEach(e -> {System.out.println(e);});
